...
class Pagination {
  ...
  factory Pagination.fromJson(Map<String, dynamic> json) {
    return Pagination(json['next'] ?? '', json['prev'] ?? '',
        json['more'] ?? '', json['max'] ?? '');
  }
}
...
class Result {
  ...
  Result(
      this.hash, this.dateTime, this.elector, this.elected, this.blockheight);

  factory Result.fromJson(Map<String, dynamic> json) {
    return Result(json['Hash'] ?? '', json['Date'] ?? '', json['Elector'] ?? '',
        json['Elected'] ?? '', json['BlockHeight'] ?? '');
  }
}
