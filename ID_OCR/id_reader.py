...
def id_read(img, gray=False, *, data, face):
    ...
    #bi_img = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 201, 30)
    ...
    for coord in data[face].values():
        bi_img = cv2.rectangle(bi_img, coord[0], coord[1], (0, 0, 0), 2)


    for key, val in data[face].items():
        (x1, y1), (x2, y2) = val
...
def get_contours(img_edges):
    contours = cv2.findContours(img_edges, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[0]
